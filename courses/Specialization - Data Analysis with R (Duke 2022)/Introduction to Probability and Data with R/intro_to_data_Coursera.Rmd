...
mean(sfo_feb_flights$arr_delay <= 0)
```
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(med_arr_delay = median(arr_delay), iqr_arr_delay = IQR(arr_delay), n = n())
...
nycflights <- nycflights %>%
  ...
  arrange(desc(avg_speed))
head(nycflights)
nycflights_dep_delayed  <- nycflights %>% filter(dep_delay > 0)
mean(nycflights_dep_delayed$arr_type == 'on time')
```