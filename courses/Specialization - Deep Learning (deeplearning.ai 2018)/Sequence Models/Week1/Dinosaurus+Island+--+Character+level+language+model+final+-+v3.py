...
def clip(gradients, maxValue):
    ...
    for key in gradients: #[dWax, dWaa, dWya, db, dby]:
        gradients[key] = np.clip(gradients[key], a_min=-maxValue, a_max=maxValue)
    ### END CODE HERE ###
...
def sample(parameters, char_to_ix, seed):
    ...
    x = np.zeros((vocab_size,1)) #to_categorical(0, vocab_size).T
    ...
    while (idx != newline_character and counter != 50):
        ...
        #print(Wax.shape, x.shape, Waa.shape, a_prev.shape)
        a = np.tanh(np.matmul(Wax, x) + np.matmul(Waa, a_prev) + b)
        z = np.matmul(Wya, a) + by
        y = softmax(z)
        #print(a.shape, y.shape, z.shape)
        ...
        #x = to_categorical(idx, vocab_size).T
