...
public class PCFGParserTester {
  ...
  public static class PCFGParser implements Parser {
    ...
    List<ArrayList<HashMap<String, Double>>> scoreTable = new ArrayList<ArrayList<HashMap<String, Double>>>(); 

    List<ArrayList<HashMap<String, String>>> backTable = new ArrayList<ArrayList<HashMap<String, String>>>(); 

    void CKY(List<String> words, Grammar grammar) {

	int nWords = words.size();

	for (int i = 0; i < nWords + 1; ++i) {
	  scoreTable.add(new ArrayList<HashMap<String, Double>>());
	  backTable.add(new ArrayList<HashMap<String, String>>());
	}		
	for (int i = 0; i < nWords + 1; ++i) {
	  ArrayList<HashMap<String, Double>> list = scoreTable.get(i);
	  for (int j = 0; j < nWords + 1; ++j) {
	    list.add(new HashMap<String, Double>());
      }
	  scoreTable.set(i, list);

	  ArrayList<HashMap<String, String>> blist = backTable.get(i);
	  for (int j = 0; j < nWords + 1; ++j) {
	    blist.add(new HashMap<String, String>());
      }
	  backTable.set(i, blist);
	}		

	HashMap<String, Double> curCell;

	for (int i = 0; i < nWords; i++) {

		String word = words.get(i);
		List<String> preterms = new ArrayList(lexicon.getAllTags());
		curCell = scoreTable.get(i).get(i + 1);

      	for (String preterm : preterms) { 
			double d = lexicon.scoreTagging(word, preterm);
			// String rule = preterm + "->" + word;
			curCell.put(preterm, d);
		}

		//handle unaries
      	for (int j = 0; j < preterms.size(); ++j) { 
			String preterm = preterms.get(j);
			double prevScore = curCell.get(preterm);
			List<UnaryRule> rules = grammar.getUnaryRulesByChild(preterm);
			for (UnaryRule r : rules) {
				if (prevScore > 0) {
					String parent = r.getParent();
					double prob = r.getScore() * prevScore; 
					double prevScoreParent = curCell.get(parent) == null ? 0 : curCell.get(parent);
					if (prob > prevScoreParent) {
					  curCell.put(parent, prob);
					  preterms.add(parent);
					  backTable.get(i).get(i+1).put(parent, preterm);
					}
				}
            }
	    }
	...
	for (int span = 2; span <= nWords; ++span) {
		for (int begin = 0; begin <=  nWords - span; ++begin) {
			...
			int end = begin + span;
			curCell = scoreTable.get(begin).get(end);

			for (int split = begin + 1; split <= end - 1; ++split) {
				HashMap<String, Double> leftChildMap = scoreTable.get(begin).get(split);
			    HashMap<String, Double> rightChildMap = scoreTable.get(split).get(end);
				Iterator it = leftChildMap.entrySet().iterator();
				while (it.hasNext()) {
				  Map.Entry pairs = (Map.Entry)it.next();
				  String leftNT = pairs.getKey().toString();
				  double leftScore = (Double)pairs.getValue();
				  List<BinaryRule> rules = grammar.getBinaryRulesByLeftChild(leftNT);
				  for (BinaryRule r : rules) {
					String rightNT = r.getRightChild();
					if (rightChildMap.get(rightNT) != null) {
						double rightScore = rightChildMap.get(rightNT);
						double ruleScore = r.getScore();
						String parent = r.getParent();
						double prob = leftScore * rightScore * ruleScore;
						Double parentProb = curCell.get(parent);
						if (parentProb == null || prob > parentProb.doubleValue()) {
							curCell.put(parent, prob);
							backTable.get(begin).get(end).put(parent, String.valueOf(split) + " " + leftNT + " " + rightNT);
						}
					}
				  }
			    }
			...
			List<String> NTs = new ArrayList<String>(curCell.keySet());

			for (int j = 0; j < NTs.size(); ++j) { 
				String NT = NTs.get(j);
				double prevScore = curCell.get(NT);
				List<UnaryRule> rules = grammar.getUnaryRulesByChild(NT);
				for (UnaryRule r : rules) {
					if (prevScore > 0) {
						String parent = r.getParent();
						double prob = r.getScore() * prevScore; 
						...
						if (prob > prevScoreParent) {
							curCell.put(parent, prob);
							NTs.add(parent);
							backTable.get(begin).get(end).put(parent,NT);
						}
					}
				}
			}
	...
	/*for (int i = 0; i < nWords + 1; ++i) {
		for (int j = i + 1; j < nWords + 1; ++j) {
			HashMap<String, Double> h = scoreTable.get(i).get(j);
    ...
    public void buildParseTree(int begin, int end, String NT,
		//List<ArrayList<HashMap<String, Double>>> scoreTable, 
		Tree<String> parent, Tree<String> root,
		List<ArrayList<HashMap<String, String>>> backTable,
	    ...
	    if (parent == null) {
	      ...
	      cur = new Tree<String>(NT);
	      ArrayList<Tree<String>> children = new ArrayList<Tree<String>>(parent.getChildren());
	      children.add(cur);
	      parent.setChildren(children);
	    }
	    if (begin != end) {
			String rule = backTable.get(begin).get(end).get(NT);
			if (rule != null) {
			  String[] symbols = rule.split("\\s+");
			  if (symbols.length == 1) {
				buildParseTree(begin, end, symbols[0], 
				cur, root, backTable, sentence);
			  }	
			  else if (symbols.length == 3) {
				try {
					int split = Integer.parseInt(symbols[0]);
					buildParseTree(begin, split, symbols[1], cur, root, backTable, sentence);
					buildParseTree(split, end, symbols[2], cur, root, backTable, sentence);
				} catch (Exception e) {
					e.printStackTrace();
				}
			  }
				...
				buildParseTree(begin, begin, sentence.get(begin), cur, root, backTable, sentence);
			}	
	   }
    }
    ...
    public void train(List<Tree<String>> trainTrees) {
      ...
      for (Tree<String> trainTree : trainTrees) {
      	trainTree = TreeAnnotations.binarizeTree(trainTree);
		trainTrees.set(index, trainTree);
		++index;
      }
    ...
    public Tree<String> getBestParse(List<String> sentence) {
      ...
      buildParseTree(0, sentence.size(), "ROOT", null, root, backTable, sentence);
  ...
  public static class TreeAnnotations {
    ...
    public static Tree<String> annotateTree(Tree<String> unAnnotatedTree) {
      ...
      Tree<String> t = binarizeTree(unAnnotatedTree);
      System.out.println("Here:");
      System.out.println(unAnnotatedTree.toString());
