...
public class LinearInterpolationModel implements LanguageModel {
  ...
  double lambda3 = 2.0, lambda2 = 3.0, lambda1 = 5.0, lambda = 5.0;
  
  /** Initialize your data structures in the constructor. */
  public LinearInterpolationModel(HolbrookCorpus corpus) {
  ...
  /** Takes a corpus and trains your language model (Linear Interpolation). 
  ...
  public double score(List<String> sentence) {
	...
	for(String word : sentence) { // iterate over words in the sentence
	  ...
	  if (len == 3) {
		  ...
		  probability = Math.log(lambda3 * (ftripple * 1.0) / fpair 
							   + lambda2 * (fpair * 1.0) / fprev
							   + lambda1 * (fcur + 1.0) / (totWordFreq + totWordType));
	  }
	  else if (len == 2) {
		  ...
		  fprev = freq == null ? 0 : freq.intValue();
	  	  freq = wordFreqMap.get(words.get(1));
		  fcur = freq == null ? 0 : freq.intValue();
		  probability = Math.log(lambda * (fpair * 1.0) / fprev + (1 - lambda) * (fcur + 1.0) / (totWordFreq + totWordType));
	  }
	  else if (len == 1) {
	  	  freq = wordFreqMap.get(words.get(0));
		  fcur = freq == null ? 0 : freq.intValue();
